...
import numpy as np
from models_origin.KAN import KAN
...
class FreTS(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.embed_size = configs.dim_expand  # embed_size
        self.hidden_size = configs.dim_f  # hidden_size
        self.pred_len = configs.pred_len
        self.feature_size = 1  # channels
...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.frets = FreTS(args)
        self.timexer = Timexer(args)
        self.patch_num = int(args.seq_len // args.patch_len) + 1
        self.kan = KAN([args.d_model, args.pred_len])
        self.gate = GTUFusion(args.d_model * self.patch_num, args.dim_f, args.d_model)

    def forward(self, x_enc, x_mark_enc=None, x_dec=None, x_mark_dec=None):
        x_t = self.timexer(x_enc)
        x_f = self.frets(x_enc[:, :, 0, None]).transpose(2,1)
        x_g = self.gate(x_t, x_f)
        x_o = self.kan(x_g).flatten(1)
        return x_o
