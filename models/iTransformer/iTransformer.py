...
class iTransformer_single(Module):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self.num_variates = enc_in
        self.lookback_len = seq_len
        self.prob=prob
        self.mem_tokens = nn.Parameter(torch.randn(num_mem_tokens, d_model)) if num_mem_tokens > 0 else None

        pred_length = cast_tuple(pred_len)
        self.pred_length = pred_length

        self.reversible_instance_norm = RevIN(enc_in, affine = reversible_instance_norm_affine) if use_reversible_instance_norm else None
        self.num_tokens_per_variate = num_tokens_per_variate

        self.layers = ModuleList([])
        for _ in range(e_layers):
            self.layers.append(ModuleList([
                Attention(d_model, dim_head = dim_head, heads = n_heads, dropout = attn_dropout, flash = flash_attn),
                nn.LayerNorm(d_model),
                FeedForward(d_model, mult = ff_mult, dropout = ff_dropout),
                nn.LayerNorm(d_model)
            ]))
        self.mlp_in = nn.Sequential(
            nn.Linear(seq_len, d_model * num_tokens_per_variate),
            Rearrange('b v (n d) -> b (v n) d', n = num_tokens_per_variate),
            nn.LayerNorm(d_model)
        )
        self.pred_heads = ModuleList([])
        for one_pred_length in pred_length:
            if not self.prob:
                head = nn.Sequential(
                    Rearrange('b (v n) d -> b v (n d)', n = num_tokens_per_variate),
                    nn.Linear(d_model * num_tokens_per_variate, one_pred_length),
                    Rearrange('b v n -> b n v')
                )
            else:
                head = nn.Sequential(
                    Rearrange('b (v n) d -> b v (n d)', n = num_tokens_per_variate),
                    nn.Linear(d_model * num_tokens_per_variate, one_pred_length*7),
                    Rearrange('b v n -> b n v')
                )
            self.pred_heads.append(head)
